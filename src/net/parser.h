...
struct operation {
    ...
    union {
        /**
         * If `ty` is something that requires a full movie definition (e.g. ADD_MOVIE),
         * this is a pointer to a newly allocated `struct movie`.
         */
        struct movie *NONNULL movie;

        /**
         * If `ty` is an operation that just needs an ID and/or a genre,
         * store them here in `movie_id` and `genre`.
         */
        struct movie_key {
            int64_t movie_id;
            char *NULLABLE genre;
 ...
 *
 * If we are inside a mapping, we expect a key that identifies the operation type, and then
 * we parse its parameters. If we are outside, we only allow certain operation types (e.g., list).
 *
